...
import exp.converter.AudioConverter;
...
extends JFrame {
    ...
    public ConverterFrame() {
        ...
        this.getContentPane().setLayout((LayoutManager)new MigLayout("insets 14, gapx 8, gapy 6", "[45px][2px][14px][2px][34px][1px][33px][222px][71px]", "[14px][23px][2px][14px][23px][14px][23px][2px][14px][14px]"));
        this.getContentPane().add((Component)separator_1, "cell 0 7 9 1,growx,aligny top");
        this.getContentPane().add((Component)separator, "cell 0 2 9 1,growx,aligny top");
        this.getContentPane().add((Component)lblInputFile, "cell 0 0 3 1,alignx left,aligny top");
        this.getContentPane().add((Component)this.inputFileLabel, "cell 4 0 3 1,alignx right,aligny top");
        this.getContentPane().add((Component)selectInputButton, "cell 0 1 3 1,alignx left,aligny top");
        this.getContentPane().add((Component)lblOutputFile, "cell 0 3 3 1,alignx left,aligny top");
        this.getContentPane().add((Component)this.outputFileLabel, "cell 4 3 3 1,alignx left,aligny top");
        this.getContentPane().add((Component)this.changeOutputButton, "cell 0 4 5 1,alignx left,aligny top");
        this.getContentPane().add((Component)lblOutputFormat, "cell 0 5 5 1,alignx left,aligny top");
        this.getContentPane().add((Component)this.mp3RadioButton, "cell 0 6,alignx left,aligny top");
        this.getContentPane().add((Component)this.wavRadioButton, "cell 2 6 3 1,alignx left,aligny top");
        this.getContentPane().add((Component)this.convertButton, "cell 8 6,alignx left,aligny top");
        this.getContentPane().add((Component)lblEncodingProgress, "cell 0 8 5 1,alignx right,aligny top");
        this.getContentPane().add((Component)this.encodingProgressLabel, "cell 6 8,alignx left,aligny top");
        this.getContentPane().add((Component)lblEncodingMessages, "cell 0 9 5 1,alignx right,aligny top");
        this.getContentPane().add((Component)this.encodingMessageLabel, "cell 6 9,alignx left,aligny top");
        this.pack();
    ...
    private void convertAction() {
        ...
        if (cont && this.outputfile.exists() && (session = JOptionPane.showConfirmDialog(null, "Output file already exists. Overwrite?", "Overwrite confirmation", 1)) != 0) {
            cont = false;
        }
        if (cont) {
            if (this.mp3RadioButton.isSelected()) {
                if (this.inputfiles.length > 1) {
                    AudioConverter.batchConvertToMP3(this.inputfiles, this.outputfile, new ConvertProgressListener());
                } else {
                    AudioConverter.convertToMP3(this.inputfiles[0], this.outputfile, new ConvertProgressListener());
                }
            } else if (this.wavRadioButton.isSelected()) {
                if (this.inputfiles.length > 1) {
                    AudioConverter.batchConvertToWAV(this.inputfiles, this.outputfile, new ConvertProgressListener());
                } else {
                    AudioConverter.convertToWAV(this.inputfiles[0], this.outputfile, new ConvertProgressListener());
                }
            }
            this.convertButton.setEnabled(false);
        }
    ...
    private void renameOutputForFormat() {
        if (this.inputfiles.length > 1) {
            if (!this.outputfile.isDirectory()) {
                ...
                String output = String.valueOf(inputabs.substring(0, slash + 1)) + "Converted";
            ...
            int period = outputfileabs.lastIndexOf(46);
            if (period > 0) {
                outputfileabs = outputfileabs.substring(0, period);
                if (this.mp3RadioButton.isSelected()) {
                    outputfileabs = String.valueOf(outputfileabs) + ".mp3";
                } else if (this.wavRadioButton.isSelected()) {
                    outputfileabs = String.valueOf(outputfileabs) + ".wav";
                }
            }
    ...
    implements EncoderProgressListener {
        ...
        public void message(String m) {
            if (ConverterFrame.this.inputfiles.length > 1 && this.current < ConverterFrame.this.inputfiles.length) {
                ConverterFrame.this.encodingMessageLabel.setText(String.valueOf(this.current) + "/" + ConverterFrame.this.inputfiles.length);
            }
        }

        public void progress(int p) {
            float progress = p / 10;
            ConverterFrame.this.encodingProgressLabel.setText(String.valueOf(progress) + "%");
            if (p >= 1000) {
                if (ConverterFrame.this.inputfiles.length > 1) {
                    ++this.current;
